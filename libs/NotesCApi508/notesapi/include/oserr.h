...
#define	ERR_MARKERS_PRECEED	PKG_OS+50
	debugtext(ERR_MARKERS_PRECEED,	"CheckMarkers: Memory preceeding object has been destroyed")
...
#if defined(NT) && !defined(_ALPHA_)
	errortext(ERR_SETUP_NOT_DONE,"Launching the Domino setup client to setup your server...")
#else
	errortext(ERR_SETUP_NOT_DONE,"Launching the Domino HTTP server. Connect to it on port 8081 with your browser to setup your Domino server...")
#endif
...
#define ERR_MAXDPOOLS_EXCEEDED			PKG_OS+178
	errortext(ERR_MAXDPOOLS_EXCEEDED,	"Exceeded maximum limit of 300MB of shared memory")
...
#define ERR_NT_ACCESS_DENIED			PKG_OS+219 
	stringtext(ERR_NT_ACCESS_DENIED, "You must be an NT Administrator or Account Operator to delete or rename NT users.")
#define ERR_NT_DC_NOT_FOUND				PKG_OS+220
	stringtext(ERR_NT_DC_NOT_FOUND, "NT user accounts cannot be deleted or renamed.  A primary domain controller for the current NT domain cannot be found.")
#define ERR_NT_DLL_NOT_FOUND			PKG_OS+221
	stringtext(ERR_NT_DLL_NOT_FOUND, "Netapi32.dll could not be found.  If available, place it in the Windows system directory (SYSTEM32).")
#define ERR_NT_USER_GROUP_ABSENT		PKG_OS+222
	stringtext(ERR_NT_USER_GROUP_ABSENT, "NT account for '%s' not deleted or renamed.  The NT user or NT group could not be found on the current NT domain.")
#define ERR_NT_MODIFY_FAILED			PKG_OS+223
	stringtext(ERR_NT_MODIFY_FAILED, "NT user '%s' could not be deleted or renamed.")
...
#define ERR_LINK_DEST_NO_EXIST			PKG_OS+249
	errortext(ERR_LINK_DEST_NO_EXIST,	"%s does not exist")
#define ERR_SERVER_MEMORY				PKG_OS+250
	errortext(ERR_SERVER_MEMORY,		"Insufficient memory.")

#define STR_OS_INTL_DSTLAW				PKG_OS+251
	stringtext(STR_OS_INTL_DSTLAW, 		"3 -1 1 10 -1 1")
#define STR_OS_INTL_TIMEZONE			PKG_OS+252
	stringtext(STR_OS_INTL_TIMEZONE, 	"0")
/*
International currecystring was stored in raw value which is over ASCII range.
And that causes the compilation problem under Non-ASCII environment. 
It is changed to hex representation to avoid that.
11/06/2000 Masaru Nakagawa
*/
#define STR_OS_INTL_CURRENCYSTRING		PKG_OS+253
	stringtext(STR_OS_INTL_CURRENCYSTRING, "\x9c")
/* PKG_OS goes to 255 */
