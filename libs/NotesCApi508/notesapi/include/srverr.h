...
/* PKG_SERVER+0 available and unused */
...
#define ERR_NO_SERVER_NAME			PKG_SERVER+56
	errortext(ERR_NO_SERVER_NAME,	"Name & Address database does not contain a server entry for this server")
#define ERR_NO_NETWORK_NAME			PKG_SERVER+57
	errortext(ERR_NO_NETWORK_NAME,	"Server entry in Name & Address database does not contain a network name (or one for this port)")
...
#define	ERR_HELP_CMD_BROADCAST		PKG_SERVER+85
	stringtext(ERR_HELP_CMD_BROADCAST,"Broadcast a message to user(s) of this server")
...
#ifdef MAC
	stringtext(ERR_CMD_SET_CONFIG,"CONFIGURATION \042variable=value\042")
#else
	stringtext(ERR_CMD_SET_CONFIG,"CONFIGURATION ""variable=value""")
#endif
...
#define ERR_CMD_SHOW_TASKS			PKG_SERVER+145
	stringtext(ERR_CMD_SHOW_TASKS,"TASKS")
...
#ifdef MAC
	stringtext(ERR_CMD_CMD_BROADCAST,"BROADCAST \042msg\042 [\042user\042]")
#else
	stringtext(ERR_CMD_CMD_BROADCAST,"BROADCAST ""msg"" [""user""]")
#endif
...
#define ERR_CMD_CMD_REPLICATE		PKG_SERVER+158
	stringtext(ERR_CMD_CMD_REPLICATE,"REPLICATE servername")
#define ERR_CMD_CMD_PULL			PKG_SERVER+159
	stringtext(ERR_CMD_CMD_PULL,"PULL servername")
...
#define ERR_MAIL_STATS				PKG_SERVER+169
	stringtext(ERR_MAIL_STATS,		 "Pending mail:  %lu        Dead mail:  %lu%n")
...
#define	ERR_MSG_PERF				PKG_SERVER+174
	stringtext(ERR_MSG_PERF,		"%s%d Transactions/Minute, %d Users")
...
#define ERR_NEED_UPGRADE			PKG_SERVER+187
	errortext(ERR_NEED_UPGRADE,		"This server does not support the requested function.  Please upgrade your server to the latest version of Notes.")
...
#define ERR_CMD_QUIT_NEEDS_PASSWORD	PKG_SERVER+192
	stringtext(ERR_CMD_QUIT_NEEDS_PASSWORD,	"This server requires a console password. Enter Exit SECURE [password] or Quit SECURE [Password].")
...
#define ERR_SRV_UNABLE_TO_FIND_PORT	PKG_SERVER+195
...
#define ERR_SRV_SETUP_GET_ID_ADDR	PKG_SERVER+204
	errortext(ERR_SRV_SETUP_GET_ID_ADDR,"Getting server ID from address book on ")
...
#define ERR_SRV_SETUP_REG_ADDR		PKG_SERVER+206
	errortext(ERR_SRV_SETUP_REG_ADDR,	"Creating address book connection entry on ")
#define ERR_SRV_SETUP_DEL_ID		PKG_SERVER+207
	errortext(ERR_SRV_SETUP_DEL_ID,		"Deleting Server ID from address book on ")
#define ERR_SRV_SETUP_DEL_ID_OK		PKG_SERVER+208
	errortext(ERR_SRV_SETUP_DEL_ID_OK,	"Deleted Server ID from address book on ")
...
#define ERR_SRV_SETUP_DEL_ID_NOACCESS	PKG_SERVER+210
	errortext(ERR_SRV_SETUP_DEL_ID_NOACCESS,	"Insufficient access to delete Server ID from address book on ")
#define ERR_SRV_SETUP_DEL_ID_ERROR	PKG_SERVER+211
	errortext(ERR_SRV_SETUP_DEL_ID_ERROR,	"Error while deleting Server ID from address book on %a.%nERROR: %e%nServer ID NOT deleted from address book.")
...
#ifdef MAC
	stringtext(ERR_CMD_CMD_DROP,"DROP [\042username\042] [ALL]")
#else
	stringtext(ERR_CMD_CMD_DROP,"DROP [""username""] [ALL]")
#endif
...
#define ERR_CMD_CMD_PUSH			PKG_SERVER+231
	stringtext(ERR_CMD_CMD_PUSH,	"PUSH servername")
...
#define ERR_HELP_SHOW_DIRECTORY		PKG_SERVER+245
	stringtext(ERR_HELP_SHOW_DIRECTORY,"Directory Information")
...
#define	ERR_NO_EVENT_SCHEDULED		PKG_SERVER2+1
	errortext(ERR_NO_EVENT_SCHEDULED,"No Events are scheduled at this time.")
...
#define ERR_TASK_MSG_TRACKING_STATE	PKG_SERVER2+25
	stringtext(ERR_TASK_MSG_TRACKING_STATE, "Message Tracking:       Not Enabled|Message Tracking:       Enabled")
#define ERR_TASK_MSG_TRACKING_DISABLED_IDX 1
#define ERR_TASK_MSG_TRACKING_ENABLED_IDX  2 

#define	ERR_CLU_NOREP				PKG_SERVER2+26
	stringtext(ERR_CLU_NOREP,		"Error replicating public address book and cluster database directory to another cluster member.  Please replicate manually.")
...
#define	ERR_TASK_DESC_GENERIC		PKG_SERVER3+12
	stringtext(ERR_TASK_DESC_GENERIC, "Load Monitor is updating server load|Datbase Directory Manager Cache Refresher is checking cache")
#define ERR_TASK_DESC_LOADMON_IDX	1
#define ERR_TASK_DESC_SVDBDIR_IDX	2

#define	ERR_TASK_DESC_GENERIC_IDLE	PKG_SERVER3+13
	stringtext(ERR_TASK_DESC_GENERIC_IDLE, "Load Monitor is idle|Database Directory Manager Cache Refresher is idle")
...
#define ERR_CMD_PERF_RESET				PKG_PERFSTAT+4
	stringtext(ERR_CMD_PERF_RESET,		"RESET")
#define ERR_HELP_CMD_PERF_RESET			PKG_PERFSTAT+5
	stringtext(ERR_HELP_CMD_PERF_RESET,	"Reset Platform statistics for new session")
#define ERR_CMD_PERF_WAIT				PKG_PERFSTAT+6
	stringtext(ERR_CMD_PERF_WAIT,		"WAIT")
...
#define ERR_CMD_PERF_RUN				PKG_PERFSTAT+8
	stringtext(ERR_CMD_PERF_RUN,		"RUN")
...
#define ERR_PERFSTAT_GENERROR			(PKG_PERFSTAT+10)
	errortext(ERR_PERFSTAT_GENERROR,	"Perform: error!")
#define ERR_PERFSTAT_INITREAD			(PKG_PERFSTAT+11)
	errortext(ERR_PERFSTAT_INITREAD,	"Perform: Unable to read initialization file %s")
#define MSG_PERFSTAT_CMDDEFAULT			(PKG_PERFSTAT+12)
	errortext(MSG_PERFSTAT_CMDDEFAULT,	"Perform: Using default counters and update interval.")
#define ERR_PERFSTAT_PSCLPOOLFULL		(PKG_PERFSTAT+13)
	errortext(ERR_PERFSTAT_PSCLPOOLFULL, "Perform: PSCounterList VPool is full.")
#define ERR_PERFSTAT_CNTRINIT			(PKG_PERFSTAT+14)
	errortext(ERR_PERFSTAT_CNTRINIT,	"Perform: Unable to set up performance counters.")
#define ERR_PERFSTAT_CNTRUPDATE			(PKG_PERFSTAT+15)
	errortext(ERR_PERFSTAT_CNTRUPDATE,	"Perform: Unable to update performance counters.")
#define ERR_PERFSTAT_CREATECNTR			(PKG_PERFSTAT+16)
	errortext(ERR_PERFSTAT_CREATECNTR,	"Perform: Unable to create counter %s of context %s.")
#define ERR_PERFSTAT_INTRVLSET			(PKG_PERFSTAT+17)
	errortext(ERR_PERFSTAT_INTRVLSET,	"Perform: Update Interval is already set to %s.")
#define ERR_PERFSTAT_INTRVLINC			(PKG_PERFSTAT+18)
	errortext(ERR_PERFSTAT_INTRVLINC,	"Perform: Update Interval command is incomplete.")
#define ERR_PERFSTAT_INTRVLBAD			(PKG_PERFSTAT+19)
	errortext(ERR_PERFSTAT_INTRVLBAD,	"Perform: Bad Update Interval %s. Must be an integer between 1 and %s.")
#define ERR_PERFSTAT_NOTINST			(PKG_PERFSTAT+20)
	errortext(ERR_PERFSTAT_NOTINST,	"Perform: %s is not a valid instance of %s.")
#define ERR_PERFSTAT_NOTSTAT			(PKG_PERFSTAT+21)
	errortext(ERR_PERFSTAT_NOTSTAT,	"Perform: %s is not a valid statistic of %s.")
#define ERR_PERFSTAT_NONODE				(PKG_PERFSTAT+22)
	errortext(ERR_PERFSTAT_NONODE,	"Perform: Unable to monitor statistic %s of context %s.")
#define ERR_PERFSTAT_UNKWNCMD			(PKG_PERFSTAT+23)
	errortext(ERR_PERFSTAT_UNKWNCMD,	"Perform: Unknown command starting with: %s.")
#define ERR_PERFSTAT_UNKNWNOPER			(PKG_PERFSTAT+24)
	errortext(ERR_PERFSTAT_UNKNWNOPER,	"Perform: Unknown operation: %s.")
#define ERR_PERFSTAT_NTPOOLFULL			(PKG_PERFSTAT+25)
	errortext(ERR_PERFSTAT_NTPOOLFULL,	"Perform: Unable to add to Perform name table.")
#define MSG_PERFSTAT_CMDTIME				PKG_PERFSTAT+26
	stringtext(MSG_PERFSTAT_CMDTIME,		"Perform: Set update interval to %s minutes.")
#define MSG_PERFSTAT_CMDTIME_DSPLY			PKG_PERFSTAT+27
	stringtext(MSG_PERFSTAT_CMDTIME_DSPLY,	"Perform: Update interval is currently %s minutes.")
#define MSG_PERFSTAT_CMDRESET				PKG_PERFSTAT+28
	stringtext(MSG_PERFSTAT_CMDRESET,		"Perform: Reinitialized statistics for new session.")
#define MSG_PERFSTAT_CMDWAIT				PKG_PERFSTAT+29
	stringtext(MSG_PERFSTAT_CMDWAIT,		"Perform: Halted statistics monitoring session.")
#define MSG_PERFSTAT_CMDRUN					PKG_PERFSTAT+30
	stringtext(MSG_PERFSTAT_CMDRUN,			"Perform: Resumed statistics monitoring session.")
