...
On platforms other than Macintosh, you must supply command-line parameters 
to specify the file pathname and, optionally, the server for the database.  
...
- Under OS/2, Windows NT, Windows 95 and UNIX (Solaris, AIX, OS/390 or HP-UX):
  Type 'rtattach' at the system command prompt, followed by the Notes
...
"Here are the attachments" followed by 
"RT Attachment goes here --->>", followed by the attachment itself and 
the text "<<--- End of RTAttachment".
On the next line there should be a comment
RT named Attachment goes here --->>" immediately followed by
"<<--- End of RT named Attachment," since the attachment itself was deleted.
