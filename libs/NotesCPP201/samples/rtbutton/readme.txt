...
rtbutton.def - Module definition (.def) file for 16-bit Borland C++ for 
	       Windows.
...
database (.nsf) file and server path names. On platforms other than the 
Macintosh PowerPC, you can also specify these parameters as command line 
arguments.

- Under Windows 16-bit, choose File - Run from the Program Manager menu, 
  type the full program path and filename, add command line arguments
  if desired, and press Enter.

- Under OS/2, Windows NT, Windows 95, and UNIX (Solaris, AIX, OS/390 or HP-UX), 
  type 'rtbutton' at the system command prompt and add command line
  arguments if desired.

- Under MacOS, double-click the program icon.
