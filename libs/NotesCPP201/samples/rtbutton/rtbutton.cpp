					...
					LNDocument *CurDoc, LNRichText *RT)
{
	...
	if (Error = NSFFormulaCompile( NULL, 0, Formula.GetBuf(),
								   (WORD)Formula.GetBufLength(),
	...
	// NOTE for C API programmers: The LNCompositeData class will make sure 
	// that the CD record is aligned on even byte boundaries for you.
	Length = (WORD)ButtonTitle.GetBufLength();
	...
	CD.Write( Length,  ButtonTitle.GetBuf(), ODSLength(_CDBUTTON) );
	...
	if (Lnstatus != LNWARN_NOT_FOUND)
	{
		while (Counter != 2) 
		{
			...
			{	 
				...
				// appropriately
				NewLength = (NewLabel.GetBufLength());
				...
				// Write the button title text after the CDButton record info.
				CD.Write( NewLength,  NewLabel.GetBuf(), ODSLength(_CDBUTTON));
