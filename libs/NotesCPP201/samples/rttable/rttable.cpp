...
int main(int argc, char *argv[])
{
	...
	try
	{
		...
		// Create the "Body" field in the note
...
void AppendRTTable( LNDocument *NewDoc, LNINT Rows, LNINT Cols )
{
	...
	// We will append the table to it.
	NewDoc->GetItem("Body", &RT);

	// Once the bug is fixed, we want to create a section that contains this table.
	// So we need to get a hold of the cursor before the table.
	...
	// Get a cursor at the end of this RT object.
	RT.GetEndCursor(&Cursor1);
	
	// Position the cursor back at the point between the two strings above.
	Cursor1-=12;

	// Create a new table just after the cursor position.
	...
	// Get the cursor after the table
	RT.GetEndCursor(&SectionEndCursor);
	SectionTitle = "Here is the table:";

	//This doesn't work so far. Filed an SPR.
	//RT.CreateSection(SectionTitle, &SectionBeginCursor, &SectionEndCursor);
						...
						LNINT		*Cols   )
{
	...
	// Not a command line program on the Macintosh.
	#if defined(MAC)
	Prompt = TRUE;
	#else
	if (argc < 2)
