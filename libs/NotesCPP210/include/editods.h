...
typedef struct
	...
	WORD	URLLength;			/* Length of the image URL that follows */
	DWORD	Flags;				/* Native image specific flags */
	DWORD	Reserved;			/* Reserved for future use */
	/*	Variable length string follows (not null terminated).  This
		string is a URL that tells us where to find the image. */
	} CDNATIVEIMAGE;
...
#define HOTSPOTREC_RUNFLAG_ODSMASK		0x007FFFFCL	/*	Mask for bits stored on disk*/
...
typedef struct
	...
	DWORD 		Flags;	
	WORD 		TargetFrameLength;
	WORD		StartDateTimeItemFormulaLength;
	WORD 		DurationItemFormulaLength;
	WORD		ReqPeopleItemsFormulaLength;
	COLOR_VALUE	BusyTimeColor;
	COLOR_VALUE FreeTimeColor;
	COLOR_VALUE	NoDataColor;
	COLOR_VALUE DataRestrictedColor;
	COLOR_VALUE GridLineColor;

	/* Yes, it's alot of extra space but there are plans for post V5 to morph
	   this control into a replacement for the freetime dialog and when that 
	   happens, we'll be needing the extra space. */

	WORD 		SpareWORD[5];
	DWORD		SpareDWORD[15];
	/* Target Frame Name (if any) */
	/* StartDateTimeItem formula (if any) */
	/* DurationItem formula (if any) */
...
typedef struct
	{
	WSIG	Header;
	BYTE	Reserved[8];	/* FOr future types/versioning perhaps...*/
	} CDLSOBJECT;
