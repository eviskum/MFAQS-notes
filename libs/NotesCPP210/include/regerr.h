...
#define STR_REG_OPENSPECIAL		   				(PKG_REG+171)
	stringtext(STR_REG_OPENSPECIAL,				"NotesNIC")
#define STR_REG_NOTESNIC						(PKG_REG+172)
	stringtext(STR_REG_NOTESNIC,				"CN=Home/OU=Notes/O=NET, welcome.nsf, Notes NIC Welcome, Notes Network Information Center on the Internet")
...
#define	ERR_REG_UPDATE_USER_SHORTNAME_PROMPT	(PKG_REG+253)
	errortext(ERR_REG_UPDATE_USER_SHORTNAME_PROMPT,	"An person entry with the short name '%s' is already in the Domino Directory. Update the entry?")
...
/* Last available error is PKG_REG+256 */
#endif
